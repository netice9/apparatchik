...
```bash
docker run -v /var/run/docker.sock:/var/run/docker.sock  -p 8080:8080 --name=apparatchik -d netice9/apparatchik:0.6.0
```
...
```bash
docker run -v /var/run/docker.sock:/var/run/docker.sock  -p 8080:8080 -e AUTH_USERNAME=admin -e AUTH_PASSWORD=somepassword --name=apparatchik -d netice9/apparatchik:0.6.0
```
